...
const DetailDiary = () => {
  ...
  return (
    <div className="mx-auto">
      <div
        className='text-lg flex px-3 py-2 items-center gap-5 dark:text-white text-black border-b border-gray-100 dark:border-gray-800'>
        <FaArrowLeft className="cursor-pointer" onClick={() => navigate(-1)}/>
        <span>{myProfile?.id === diaryContents?.user_id.id ? '나' : `${diaryContents?.user_id.nickname}`}의 일기</span>
      </div>
      <div className="px-6 py-3 mt-2 space-y-3">
        <div className="flex items-center justify-between">
          <button className="flex items-center gap-3"
            ...
            <div>
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">{date} 작성</div>
            </div>
          </button>
        ...
        <div className="mt-4 pb-2 border-b border-gray-100 dark:border-gray-800">
          ...
          {image_url &&
            <img src={image_url} className='size-36'/>
          }
        </div>
